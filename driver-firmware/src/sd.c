...
enum sd_speed { SD_SPEED_400KHZ, SD_SPEED_25MHZ };
...
static int sd_init(hwif *hw)
{
	...
	if (r != 0x01 && !(r & 0x4)) { /* allow it to not be implemented - old cards */
		...
		for (j=4; j<=23; j++)
			if (r3 & 1<<j)
				break;
		printf("Vdd voltage window: %i.%i-", (12+j)/10, (12+j)%10);
		for (j=23; j>=4; j--)
			if (r3 & 1<<j)
				break;
...
static int sd_read(hwif* hw, u32 address, u8 *buf)
{
	int r;
	int tries = 10;
...
static int sd_write(hwif* hw, u32 address,const u8 *buf)
{
	int r;
	int tries = 10;
